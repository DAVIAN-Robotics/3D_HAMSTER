...
class DinoVisionTransformer(nn.Module):
    ...
    def interpolate_pos_encoding(self, x, h, w):
        ...
        if not self.onnx_compatible_mode and npatch == N and w == h:
            return self.pos_embed.to(dtype=previous_dtype)  # Ensure dtype matches
    ...
    def interpolate_pos_encoding_without_cls(self, x, h, w, input_pos_embed):
        ...
        if not self.onnx_compatible_mode and npatch == N and w == h:
            return input_pos_embed.to(dtype=previous_dtype)  # Ensure dtype matches
        ...
        assert (h0, w0) == patch_pos_embed.shape[-2:]
        patch_pos_embed = patch_pos_embed.permute(0, 2, 3, 1).flatten(1, 2)
        # Convert back to original dtype to match input tensor dtype (important for bfloat16)
        return patch_pos_embed.to(dtype=previous_dtype)
    ...
    def _get_intermediate_layers_not_chunked(self, x_img, x_depth, x_img_mask=None, x_depth_mask=None, n=1, return_mae_aux=False, **kwargs):
        ...
        # Convert list to tensor to avoid xFormers requirement for nested tensors
        # If x is a list (from prepare_tokens_with_masks), concatenate it
        ...
        # Split back to per-sample tensors if needed (for compatibility)
        # Note: This maintains the original behavior but avoids nested tensor requirement
        if kwargs.get('enable_depth_mask', True):
            # Split the concatenated tensor back into per-sample tensors
            B = len(x_img) if isinstance(x_img, list) else x_img.shape[0]
